	...
	for (i = 0; i < dfetch->num_read; i++) {
		...
		if (dfetch->dfetch_index >= dfetch->dfetch_size &&
			...
			struct dfetch_pair *temp =
				...
				GFP_KERNEL);
			dfetch->dfetch_pairs = temp ? temp : dfetch->dfetch_pairs;
			dfetch->dfetch_size =
			    temp ? dfetch->dfetch_size * 2 : dfetch->dfetch_size;
		}
		if (dfetch->dfetch_index < dfetch->dfetch_size) {
...
static void report(void)
{
	...
	for (i = 0; i < current->dfetch.dfetch_index; i++) {
		...
		pr_err(
		    "First %px len %lu Caller %lu \nSecond %px len %lu Caller "
		    "%lu \n \n",
		    current->dfetch
			.dfetch_addresses[current->dfetch.dfetch_pairs[i].first].start_address,
		    current->dfetch
			.dfetch_addresses[current->dfetch.dfetch_pairs[i].first].len,
		    current->dfetch
			.dfetch_addresses[current->dfetch.dfetch_pairs[i].first].caller,
		    current->dfetch
			.dfetch_addresses[current->dfetch.dfetch_pairs[i].second].start_address,
		    current->dfetch
			.dfetch_addresses[current->dfetch.dfetch_pairs[i].second].len,
		    current->dfetch
			.dfetch_addresses[current->dfetch.dfetch_pairs[i].second].caller);
		pr_err("============================================================\n");
		if (panic_on_warn) {
			panic_on_warn = 0;
