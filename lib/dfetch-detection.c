...
static bool is_intersect(struct dfetch_address_range a,
...
{
	int i;
	struct double_fetch *dfetch = &current->dfetch;

	for (i = 0; i < dfetch->num_read; i++) {
		if (!is_intersect(dfetch->dfetch_addresses[i],
				  dfetch->dfetch_addresses[dfetch->num_read],
				  kernel_addr))
			continue;
		if (dfetch->dfetch_index >= dfetch->dfetch_size &&
		    dfetch->dfetch_size <
			    DFETCH_MAX_RECORDS * DFETCH_MAX_RECORDS) {
			struct dfetch_pair *temp =
				(struct dfetch_pair *)krealloc(
					dfetch->dfetch_pairs,
					dfetch->dfetch_size * 2 *
						sizeof(struct dfetch_pair),
					GFP_KERNEL);
			dfetch->dfetch_pairs =
				temp ? temp : dfetch->dfetch_pairs;
			dfetch->dfetch_size = temp ? dfetch->dfetch_size * 2 :
						     dfetch->dfetch_size;
		}
		if (dfetch->dfetch_index < dfetch->dfetch_size) {
			dfetch->dfetch_pairs[dfetch->dfetch_index].first = i;
			dfetch->dfetch_pairs[dfetch->dfetch_index].second =
				dfetch->num_read;
			dfetch->dfetch_index++;
		}
	}
...
static void report(void)
{
	...
	struct dfetch_address_range first_fetch, second_fetch;

	for (i = 0; i < current->dfetch.dfetch_index; i++) {
		first_fetch = current->dfetch.dfetch_addresses
				      [current->dfetch.dfetch_pairs[i].first];
		second_fetch = current->dfetch.dfetch_addresses
				       [current->dfetch.dfetch_pairs[i].second];
		if (first_fetch.stack && second_fetch.stack) {
			first_nr_entries = stack_depot_fetch(first_fetch.stack,
							     &first_entries);
			second_nr_entries = stack_depot_fetch(
				second_fetch.stack, &second_entries);
		...
		pr_err("First %px len %lu Caller %lu \nSecond %px len %lu Caller "
		       "%lu \n \n",
		       first_fetch.start_address, first_fetch.len,
		       first_fetch.caller, second_fetch.start_address,
		       second_fetch.len, second_fetch.caller);
		pr_err("============================================================\n");
		if (panic_on_warn) {
			panic_on_warn = 0;
...
{
	unsigned long entries[DFETCH_STACK_DEPTH];
			...
			void *kernel_addr)
{
	if (!in_task() || current->dfetch.dfetch_addresses == NULL ||
		...
		return;
	if (len > DFETCH_MAX_LEN || addr == 0 || addr > TASK_SIZE)
		return;
